...
function getHourlyHeatmapWeekday(restaurantId, month, year, statuses = [BOOKING_STATUSES.CONFIRMED, BOOKING_STATUSES.COMPLETED]) {
    const params = validateAnalyticsParams(restaurantId, month, year);
...
function getBookingMetrics(restaurantId, month, year) {
    ...
    const queries = [
        dbAll(`SELECT COUNT(*) as count FROM bookings WHERE fkRestaurantId = ? AND strftime('%Y-%m', bookingDate) = ?`, 
              [restaurantId, yearMonth]),
        dbAll(`SELECT COUNT(*) as count FROM bookings WHERE fkRestaurantId = ? AND strftime('%Y-%m', bookingDate) = ? AND status = ?`, 
