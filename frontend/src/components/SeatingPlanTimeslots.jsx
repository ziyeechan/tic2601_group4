...
export function SeatingPlanTimeSlots({
  ...
  return (
    <Card className="mb-lg">
      <Card.Header title="Today's Time Slots" />
      <Card.Content>
        {timeSlots.length === 0 ? (
          ...
          >
            {timeSlots.map((slot) => {
              ...
              const tooltipLines = [`${slot.start} – ${slot.end}`, `Status: ${status}`];
              if (booking) {
                tooltipLines.push(
        ...
        >
          ...
          <br />• Click an <strong>available</strong> slot to mark it unavailable, click again to
          reset.
          <br />
