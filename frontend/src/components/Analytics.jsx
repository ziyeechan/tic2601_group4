...
export function Analytics({ onCheck }) {
  ...
  const handleCheck = async () => {
    ...
    try {
      ...
      setDailyCount(
        Array.isArray(dcJson.dailyBookingCount) ? dcJson.dailyBookingCount : []
      );
      setMetrics(mJson.metrics || null);
      setHeatmap(hmJson.heatmap || null);
      setDailyRatings(
        Array.isArray(drJson.dailyAverageRating)
          ? drJson.dailyAverageRating
          : []
      );
    } catch (e) {
  ...
  const completedPct = useMemo(
    () => pct(completed, totalBookings),
    [completed, totalBookings]
  );
  const noShowPct = useMemo(
    () => pct(noShow, totalBookings),
    [noShow, totalBookings]
  );
  const cancelledPct = useMemo(
    () => pct(cancelled, totalBookings),
      ...
      {!hasChecked && (
        <Card styles={{ padding: 'var(--spacing-lg)', textAlign: 'center' }}>
          <p style={{ color: 'var(--text-muted)', margin: 0 }}>
            Select a restaurant, year, and month, then click{' '}
            <strong>"Check"</strong> to generate details.
          </p>
        </Card>
      )}
      ...
      {hasChecked && !filtersChanged && !loading && !hasData && (
        ...
        >
          <p style={{ color: 'var(--text-muted)', marginBottom: 8 }}>
            ...
            <strong>
              {selectedRestaurant?.restaurantName || 'this restaurant'}
            </strong>{' '}
            in{' '}
            <strong>
